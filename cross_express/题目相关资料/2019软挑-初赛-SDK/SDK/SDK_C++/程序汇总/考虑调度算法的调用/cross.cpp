...
void makeCar(map<int, car, less<int>>&Car, string A,map<int, list<int>>&EachTimeOfPlan, map<int, vector<int>>&ThePathOfDrive)   //将car.txt导入到Car的Vector中。
{
	if (A[0] == '#')
		return;
	else
	{
		...
		if (EachTimeOfPlan.count(plan_time) == 0)                    //此过程为记录每个发车片段的对应的车牌号
		{
			list<int>Li;
			EachTimeOfPlan.insert(pair<int, list<int>>(plan_time, Li));
			EachTimeOfPlan[plan_time].push_back(Id);
		}
		else
			EachTimeOfPlan[plan_time].push_back(Id);
		...
		vector<int>vec;
		ThePathOfDrive.insert(pair<int,vector<int>>(Id,vec));   //为每一辆车增加一个queue，用来存储车辆的驾驶路线
		NUM_OF_CAR++;
	}
}
...
void makeNodeToPath(int *nodeToPath, map<int, road, less<int>>& Road, int num_cross, int num_road, map<int, int>&CrossToInt)   //该函数用于记录两个路口之间的道路号，用于后期将floyd算法得到的节点路径转化为道路路径
{
...
*/
void driveAllCarJustOnRoadToEndState1(int No_road, map<int, int*>Lane, map<int, car, less<int>>&Car, map<int, road, less<int>>&Road)  //处于调度算法的第一步 No_road是相对值，减去第一条路的Id
{
	...
	for (int i = 0;i < len;i++)
	{
		if (Lane[No_road][i] != -1)
		{
			int v = min(Car[Lane[No_road][i]].getmax_v(), Road[No_road].getmax_v());
			if ((i - v*num < 0 && last_one[i%num]<0) || (last_one[i%num] >= (i - v*num) && Car[Lane[No_road][last_one[i%num]]].getstate() == 4)) //说明该车出路口或者前面有等待的车挡着
			{
				Car[Lane[No_road][i]].setstate(4);   //该车状态设置为等待
				last_one[i%num] = i;
			}
			else if ((last_one[i%num] >= 0 && last_one[i%num] >= (i - v*num)) && Car[Lane[No_road][last_one[i%num]]].getstate() == 5)  //该车当前车道前面的车已调度完，但在该车最大行使距离内
			{
				Car[Lane[No_road][i]].setstate(5);   //该车状态设置为完成调度
			...
			{
				Car[Lane[No_road][i]].setstate(5);    //当该车可以行驶时，行驶，并设置为已调度完成
	...
	if (isD == 1)           //当是双车道时，需要对另一方向车道进行处理
	{
		...
		for (int i = len;i < 2 * len;i++)
		{
			...
			{
				int v = min(Car[Lane[No_road][i]].getmax_v(), Road[No_road].getmax_v());
				if ((i - v*num < len&& last_one[i%num]<len) || (last_one[i%num] >= (i - v*num) && Car[Lane[No_road][last_one[i%num]]].getstate() == 4)) //说明该车出路口或者前面有等待的车挡着
				{
					Car[Lane[No_road][i]].setstate(4);   //该车状态设置为等待
					last_one[i%num] = i;
				}
				else if ((last_one[i%num] >= len && last_one[i%num] >= (i - v*num)) && Car[Lane[No_road][last_one[i%num]]].getstate() == 5)  //该车当前车道前面的车已调度完，但在该车最大行使距离内
				{
					Car[Lane[No_road][i]].setstate(5);   //该车状态设置为完成调度
					if (i != last_one[i%num] + num)              //当该车不在前车后面一个
					{
...
int getCarId(int roadId, map<int, road, less<int>>&Road, map<int, car, less<int>>&Car, map<int, int*>Lane, int isFrom)   //该函数用来得到当前道路优先调用的等待车辆的ID，如果调用结束，则返回-1，配置相关属性
{
	...
	while (first_sch != len + len*isFrom)
	{
		if (Lane[roadId][first_sch] == -1)
			first_sch++;
		else if (Car[Lane[roadId][first_sch]].getstate() == 5)
			first_sch++;
		else
		{
...
int getCarId1(int roadId, map<int, road, less<int>>&Road, map<int, car, less<int>>&Car, map<int, int*>Lane, int isFrom)   //该函数用来得到当前道路优先调用的等待车辆的ID，如果调用结束，则返回-1，配置相关属性
{
	int first_sch = Road[roadId].getfirst_sch(isFrom);
	int lon = Road[roadId].getlongth();
	int num = Road[roadId].getnum_road();
...
int driveCar1(int carId, int No_Cross, int cur_road, int next_road, map<int, road, less<int>>&Road, map<int, car, less<int>>&Car, map<int, int*>Lane)   //该函数用于处理要过路口的车辆，更新相关参数
{
	...
	int isFromNext = No_Cross  == Road[next_road].getfromId() ? 0 : 1;  //这边判断isFrom得反一下，相对于驶入车辆其判断与是处车辆反一下
	...
	if (next_exp_d != 0)
	{
		...
		for (int i = 0;i < next_num;i++)
		{
			...
			if (Road[next_road].getlast_sch(next_num*isFromNext + i) == -1)
				loc = -1;
			if (loc == next_lon - 1 && Lane[next_road][Road[next_road].getlast_sch(next_num*isFromNext + i)] != -1 && Car[Lane[next_road][Road[next_road].getlast_sch(next_num*isFromNext + i)] - first_car].getstate() == 4)     //如果下一车道的最后一个位置有车，但是等待，当前道路的车继续等待
				return 0;
...
void driveCar2(int carId, int roadId, map<int, road, less<int>>&Road, map<int, car, less<int>>&Car, map<int, int*>Lane)   //调度那些不需要驶过路口的车
{
	int num_road = Road[roadId].getnum_road();
	int lon = Road[roadId].getlongth();
	int len = num_road*lon;
	int v = min(Car[carId].getmax_v(), Road[roadId].getmax_v());
...
bool driveCar3(int carID, map<int, car, less<int>>&Car, int No_road, map<int, road, less<int>>&Road, map<int, int*>Lane, int first_cross)  //对从车库出发的车进行调度，carID是绝对值，road是相对值
{
	int fromId = Road[No_road].getfromId();
	int crossId = Car[carID ].getfromId();
	int isFrom = crossId == fromId ? 0 : 1;   //这边判断反一下，因为该crossId是入口Id不是出口Id;
	int num_road = Road[No_road].getnum_road();
	int lon = Road[No_road].getlongth();
	int len = num_road*lon;
	int v = min(Car[carID ].getmax_v(), Road[No_road].getmax_v());   //获取该车能够在该道路上行驶的速度
...
int  driveAllCarForEachCross(int No_Cross, map<int, cross, less<int>>&Cross, map<int, road, less<int>>&Road, map<int, int*>Lane, map<int, car, less<int>>&Car, map<int,vector<int>>&ThePathOfDrive)   //此处是对于每个路口的车辆进行调度，No_Cross是相对值。
{
									 //
	int road11 = Cross[No_Cross].getroad1();
	int road21 = Cross[No_Cross].getroad2();
	int road31 = Cross[No_Cross].getroad3();
	int road41 = Cross[No_Cross].getroad4();
	int arr0[4] = { road11,road21,road31,road41 };
	int i=0;
	int kk = 100000;
	for (int j = 0;j < 4;j++)
	{
		if (arr0[j] != -1 && arr0[j] < kk)
		{
			i = j;
			kk = arr0[j];
		}
	...
	i = 3 - i;
	int road1=arr0[i%4];
	int road2=arr0[(1+i)%4];
	int road3 = arr0[(2 + i) % 4];
	int road4 = arr0[(3 + i) % 4];
	...
	{
		if (arr[i] != -1 && (Road[arr[i]].getisDouble() == 0) && (No_Cross  == Road[arr[i]].getfromId()))
			arr[i] = -1;
	}
	...
	for (int i = 0;i < 4;i++)    //判断该路口ID相对每条道路是FromID还是ToID
	{
		if (arr[i] != -1)
		{
			int is = ((No_Cross) == (Road[arr[i]].getfromId())) ? 1 : 0;
	...
	while (Cross[No_Cross].getfinish_road()<4)    //当当前路口还有道路没有调度完，否则跳出当前当前路口，进入下一个
	{
		...
		int carID = getCarId(cur_road, Road, Car, Lane, Road[cur_road].getisFrom());      //获取当前待调度的车的carID
		...
		if (Road[cur_road].getlast_finish(isFrom1*num_road1 + first_sch1%num_road1) == (len1*isFrom1 - 1))      //表示该道路上该车前面没有车挡着
		{
			...
			if (carID != -1)
			{
				int v11 = min(Car[carID ].getmax_v(), Road[cur_road].getmax_v());
				...
				else if (remain11<v11&&Car[carID ].gettoId()  == No_Cross)    //当当前车辆会驶出路口但已经行驶到目的地
				{
					NUM_OF_FINISH++;
					NUM_ON_ROAD--;
					//	cout << "该车已经完成调度" << endl;
			...
			for (pos = ThePathOfDrive[carID ].begin();pos != (ThePathOfDrive[carID ].end());pos++)
			{
				if ((*pos) == cur_road )    //搜索到当前道路
				{

					pos++;
					if (pos != (ThePathOfDrive[carID ].end()))
					{
						//cout << "  pos++" << endl;
						int next_r = (*pos) ;    //下一条要行使的道路的相对值
						if (next_r == arr1[(No + 2) % 4])   //表示直行
							Car[carID].setoperate(0);
						else if (next_r == arr1[(No + 1) % 4])   //表示左转
							Car[carID].setoperate(1);
						else if (next_r == arr1[(No + 3) % 4])   //表示左右转
							Car[carID ].setoperate(2);
						break;
					}
					else
						pos--;
			...
			//cout << "doing it " << endl;
			if ((Car[carID].getoperate()) == 0)         //表示当前的车为直行
			{
				int next_road = arr1[(No + 2) % 4];
				int isdrive = driveCar1(carID, No_Cross, cur_road, next_road, Road, Car, Lane);
			...
			else if ((Car[carID].getoperate()) == 1)     //表示当前的车为左转
			{
				...
				if (next_road1 != -1)    //考虑arr[(No + 3) % 4]这条道路是否为-1，如果不是-1进行判断
				{
					...
					int carID1 = getCarId1(next_road1, Road, Car, Lane, isFrom_next);
					if (carID1 != -1 && Car[carID1].getoperate() == 0)     //当左转遇到直行时，需要转到直行车道
					{
						No = (No + 3) % 4;
						cur_road = next_road1;
						num_of_driven = 0;
			...
			else if ((Car[carID ].getoperate()) == 2)     //表示当前的车为右转
			{
				...
				if (next_road1 != -1)
				{
					...
					int carID1 = getCarId1(next_road1, Road, Car, Lane, isFrom_next1);
					if (carID1 != -1 && Car[carID1 ].getoperate() == 0)     //当右转遇到直行时，需要转到直行车道
					{
						No = (No + 1) % 4;
						cur_road = next_road1;
						num_of_driven = 0;
				...
				if (next_road2 != -1)
				{
					...
					int carID2 = getCarId1(next_road2, Road, Car, Lane, isFrom_next2);
					if (carID2 != -1 && Car[carID2 ].getoperate() == 1)     //当右转遇到左转时，需要转到左转车道
					{
						No = (No + 2) % 4;
						cur_road = next_road2;
						num_of_driven = 0;
...
bool driveAllCarToTheEnd(map<int, cross, less<int>>&Cross, map<int, road, less<int>>&Road, map<int, int*>Lane, map<int, car, less<int>>&Car, map<int,vector<int>>&ThePathOfDrive)  //该函数对调度第二步的每个路口的遍历,如果进入死锁则返回false否则返回true
{
	...
	{
		for (pos_cross = Cross.begin();pos_cross != Cross.end();pos_cross++)    //一次对每个路口进行遍历
		{
			if (pos_cross->second.getfinish_road() == 4)          //表示该路口已经调度完成
			{
				No_cross_finish++;
				continue;
			}
			...
			{
				int num = driveAllCarForEachCross(pos_cross->second.getId(), Cross, Road, Lane, Car, ThePathOfDrive);
				if (pos_cross->second.getfinish_road() == 4)
					No_cross_finish++;
				num_driven_car += num;
			}
...
bool dispatchCarPerTime(int cur_time, map<int, cross, less<int>>&Cross, map<int, road, less<int>>&Road, map<int, int*>Lane, map<int, car, less<int>>&Car, list<int>& Ready_start, map<int,vector<int>>&ThePathOfDrive, int *minLen2Nodes1, int *minLen2Nodes2, int *pathOf2Nodes1, int *pathOf2Nodes2, int *nodeToPath)
{
	map<int, road, less<int>>::iterator pos_road;
	map<int, cross, less<int>>::iterator pos_cross;
	map<int, car, less<int>>::iterator pos_car;
	for (pos_road = Road.begin();pos_road != Road.end();pos_road++)     //调度第一步，对每一条道路进行进行调度
	{
		driveAllCarJustOnRoadToEndState1(pos_road->second.getId(), Lane, Car, Road);
	}
	//cout << "完成第一步" << endl;
	...
	{
		if (NUM_ON_ROAD > 600)   //路上车大于700不发车
			break;
		if (cur_time % 3 != 0 && cur_time != 1 && cur_time != 2)
			break;
		if (num_depart > 200)    //单个时间片发车量不超过400
			break;
		if (cur_time > 10 && num_depart > 60)   //时间超过10，单个时间片发车量不超过100
			break;
	...
	for (pos_car = Car.begin();pos_car != Car.end();pos_car++)
		pos_car->second.setstate(3);             //将每一辆车都标识为待调度
	for (pos_cross = Cross.begin();pos_cross != Cross.end();pos_cross++)
		pos_cross->second.setfinish_road(0);     //将每一个路口的finish_road重置为0
	for (pos_road = Road.begin();pos_road != Road.end();pos_road++)   //将每条道路的is_finished设置为0
	{
		int isD = pos_road->second.getisDouble();
		for (int j = 0;j < (isD + 1);j++)
			pos_road->second.setis_finished(j, 0);
	}
	return true;
}
int dispatchCar(map<int, cross, less<int>>&Cross, map<int, road, less<int>>&Road, map<int, int*>Lane, map<int, car, less<int>>&Car, map<int,vector<int>>&ThePathOfDrive, map<int, list<int>>&EachTimeOfPlan, int *minLen2Nodes1, int *minLen2Nodes2, int *pathOf2Nodes1, int *pathOf2Nodes2, int *nodeToPath)  //一个时间片一个时间片进行相应的调度
{
	int cur_time = -1;
	...
	while (NUM_OF_FINISH != NUM_OF_CAR)     //当还有车没有到达目的地，不断进行调度
	{
		...
		map<int, road, less<int>>::iterator pos_road;
		for (pos_road = Road.begin();pos_road != Road.end();pos_road++)
		{
			int num = pos_road->second.getnum_road();
			int lon = pos_road->second.getlongth();
			int isD = pos_road->second.getisDouble();
...
int main(int argc, char *argv[])
{
	...
	map<int, car, less<int>>Car;   //用来存储汽车	map<int,vector<int>>ThePathOfDrive;  //记录每辆车的行车路线，第一位记录的是其真实发车时间
	map<int, list<int>>EachTimeOfPlan;  //每个时间片准备发车的车组成的链表
	map<int,vector<int>>ThePathOfDrive;  //记录每辆车的行车路线
	...
	for (int i = 0;i < size;i++)
		nodeToPath[i] = -1;
	makeNodeToPath(nodeToPath, Road, NUM_OF_CROSS, NUM_OF_ROAD, CrossToInt);
	makeMinLen2Nodes(minLen2Nodes1, minLen2Nodes2, pathOf2Nodes1, pathOf2Nodes2, Road, NUM_OF_ROAD, NUM_OF_CROSS, CrossToInt);   //构建当前道路环境下的任意两点之间的最短路径
	for (pos_car = Car.begin();pos_car != Car.end();pos_car++ )       //为每一辆车规划静态路线  此处的carId为相对值
	{
		int from =CrossToInt[ pos_car->second.getfromId() ];
		int to = CrossToInt[pos_car->second.gettoId()];
		int v = pos_car->second.getmax_v();
		{
			if (v > MIN_V_ROAD)                 //记录行车路线
			{
				...
				while (pathOf2Nodes2[i*NUM_OF_CROSS + j] != to)
				{
					...
					ThePathOfDrive[pos_car->second.getId()].push_back(nodeToPath[i*NUM_OF_CROSS + k]);
					i = k;
				}
				ThePathOfDrive[pos_car->second.getId()].push_back(nodeToPath[i*NUM_OF_CROSS + j]);
			}
			else
			{
				int i = from;
	...
	if (value > 0)
	{
		...
		for (pos_car = Car.begin();pos_car != Car.end();pos_car++)
		{
			outfile << "(" << pos_car->second.getId() << "," << pos_car->second.getact_time();
			vector<int>::iterator pos;
			for (pos = ThePathOfDrive[pos_car->second.getId()].begin();pos != ThePathOfDrive[pos_car->second.getId()].end();pos++)
				outfile << "," << *pos;
			outfile << ")" << endl;
		}
		outfile.close();
