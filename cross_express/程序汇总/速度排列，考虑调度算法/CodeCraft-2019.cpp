...
private:
	...
	int plan_time;
	int act_time;
	int plan_depart_time;  //用于决定车是否发车，当当前车辆不能行使，将其plan_depart_time顺势延后
...
public:
	...
	int getplan_depart_time() { return plan_depart_time; }
	void setplan_depart_time(int t) { plan_depart_time = t; }
	void printCar();
};
...
void makeCar(vector<car>&Car, string A, vector<vector<int>>&ThePathOfDrive, map<int, list<int>, greater<int>>&Different_v)   //将car.txt导入到Car的Vector中。
{
	if (A[0] == '#')
		return;
	else
	{
		...
		if (Different_v.count(max_v) == 0)                    //此过程为记录每个发车片段的对应的车牌号
		{
			list<int>Li;
			Different_v.insert(pair<int, list<int>>(max_v, Li));
			Different_v[max_v].push_back(Id);
		}
		else
			Different_v[max_v].push_back(Id);
			...
			if (loc != next_lon - 1)   //表示该条道路没满
			{
				...
				if (remain >= next_exp_d)  //表示下一条道路剩余的空间大于所要行驶的空间
				{
					...
					if (isFrom == 1)                   //调整道路上的车辆数
					{
						...
						float con = (float)num / (float)cur_len;
						Road[cur_road].setcongestion2(con);
					}
					else
					{
						...
						float con = (float)num / (float)cur_len;
						Road[cur_road].setcongestion1(con);
					}
					if (isFromNext == 1)
					{
						...
						float con = (float)num / (float)next_len;
						Road[next_road].setcongestion2(con);
					}
					else
					{
						...
						float con = (float)num / (float)next_len;
						Road[next_road].setcongestion1(con);
					}
...
bool driveCar3(int carID, vector<car>&Car, int No_road, vector<road>&Road, int **Lane, int first_cross)  //对从车库出发的车进行调度，carID是绝对值，road是相对值
{
	...
	int v = min(Car[carID - first_car].getmax_v(), Road[No_road].getmax_v());   //获取该车能够在该道路上行驶的速度
																				/*
																				if (isFrom == 0)                  //此处是来判断当道路拥挤程度达到某一值时就不发车
																				{
																				if (Road[No_road].getcongestion1() >0.8)
																				{
																				//	cout << "发车号：" << carID << "道路" << No_road << "拥挤程度：" << Road[No_road].getcongestion1();
																				return false;
																				}
																				}
																				else
																				{
																				if (Road[No_road].getcongestion2() > 0.8)
																				{
																				//	cout << "发车号：" << carID << "道路" << No_road << "拥挤程度：" << Road[No_road].getcongestion2();
...
int  driveAllCarForEachCross(int No_Cross, vector<cross>&Cross, vector<road>&Road, int **Lane, vector<car>&Car, vector<vector<int>>&ThePathOfDrive)   //此处是对于每个路口的车辆进行调度，No_Cross是相对值。
{
	...
	{
		if (num_of_change == 31)   //可以有效避免节点内由于道路调度进入局部死锁，导致有些道路无法遍历的问题
		{
			No = (No + 1) % 4;
			cur_road = arr[No];
			...
			if (carID != -1)
			{
				...
				else if (remain11<v11&&Car[carID - first_car].gettoId() - first_cross == No_Cross)    //当当前车辆会驶出路口但已经行驶到目的地
				{
					...
					if (isFrom1 == 0)
					{
						...
						num--;
						Road[cur_road].setnum_car1(num);
						float con = (float)num / (float)len1;
						Road[cur_road].setcongestion1(con);
					}
					else
					{
...
bool dispatchCarPerTime(int cur_time, vector<cross>&Cross, vector<road>&Road, int **Lane, vector<car>&Car, vector<vector<int>>&ThePathOfDrive, int *minLen2Nodes1, int *minLen2Nodes2, int *pathOf2Nodes1, int *pathOf2Nodes2, int *nodeToPath, list<int> &depart_car_list, int *perTimeDepart)
{
	...
	int num_depart = 0;
	/*
	for (int carId = 0;carId < NUM_OF_CAR;carId++)
	{
	int plan_depart_time = Car[carId].getplan_depart_time();
	int act_time = Car[carId].getact_time();
	if (plan_depart_time > cur_time||act_time!=-1 )    //判定后的预计发车时间大于当前时间或者已发车就调度下一辆
	continue;
	int carID = carId + Car[0].getId();    //车牌号的绝对值
	if (NUM_ON_ROAD > 1500)   //路上车大于700不发车
	break;
	//if (cur_time > 10 && cur_time % 3 != 0)
	//	break;
	if (cur_time>10&&cur_time<800&&num_depart > 40)
	break;
	if (cur_time > 800 && num_depart > 100)
	break;
	if (num_depart > 110)
	break;
	...
	if (cur_time < 10 && cur_time % 3 != 0)
	break;
	if (cur_time >= 100 && cur_time < 350 && cur_time % 3!= 0)
	break;
	if (cur_time >= 350 && cur_time < 500 && cur_time % 5 != 0)
	break;
	if (cur_time >= 500  && cur_time % 2 != 0)
	break;
	if (cur_time > 10 && cur_time < 850 && num_depart > 50)   //时间超过10，单个时间片发车量不超过100
	break;
	if (cur_time <=10 && num_depart > 200)   //时间超过10，单个时间片发车量不超过100
	break;
	if (cur_time >850 && num_depart > 300)   //时间超过10，单个时间片发车量不超过100
	break;
	int No_road = ThePathOfDrive[carId].front() - first_road;
	bool isDepart = driveCar3(carID, Car, No_road, Road, Lane, first_cross);
	if (isDepart == true)
	{
	Car[carId].setact_time(cur_time);
	}
	else
	{
	if (cur_time < 300)
	Car[carId].setplan_depart_time(cur_time + 300);
	else if(cur_time < 800)
	Car[carId].setplan_depart_time(cur_time + 150);
	else
	Car[carId].setplan_depart_time(cur_time + 10);
	}
	}
	*/
	for (list<int>::iterator pos = depart_car_list.begin();pos != depart_car_list.end();)
	{
		/*
		if (cur_time<167&&NUM_ON_ROAD > 800)
		break;
		if (cur_time<415&&cur_time > 167 && NUM_ON_ROAD > 650)
		break;
		if (cur_time<519 && cur_time > 415 && NUM_ON_ROAD > 700)
		break;
		if ( cur_time > 519 && NUM_ON_ROAD > 700)
		break;
		*/
		//if (NUM_ON_ROAD > 700)
		//	break;
		if (cur_time<perTimeDepart[0] && NUM_ON_ROAD >700)
			break;
		if (cur_time<perTimeDepart[1] && cur_time >=perTimeDepart[0] && NUM_ON_ROAD > 720)
			break;
		if (cur_time<perTimeDepart[2] && cur_time >= perTimeDepart[1] && NUM_ON_ROAD > 800)
			break;
		if (cur_time >= perTimeDepart[2] && NUM_ON_ROAD > 950)
			break;
		if (num_depart > 200)
			break;
		if (cur_time > 10 && num_depart > 70)
			break;
		int No_road = ThePathOfDrive[*pos - first_car].front() - first_road;
		int plan_time = Car[*pos - first_car].getplan_time();
		if (plan_time > cur_time)
		{
			pos++;
			continue;
		}
...
int dispatchCar(vector<cross>&Cross, vector<road>&Road, int **Lane, vector<car>&Car, vector<vector<int>>&ThePathOfDrive, int *minLen2Nodes1, int *minLen2Nodes2, int *pathOf2Nodes1, int *pathOf2Nodes2, int *nodeToPath, map<int, list<int>, greater<int>>&Different_v)  //一个时间片一个时间片进行相应的调度
{
	int cur_time = -1;
	list<int> depart_car_list;
	int num_car_v;
	map<int, int> num_per_v;
	int cur_v = Different_v.begin()->first;
	depart_car_list.merge(Different_v[cur_v]);
	int num_of_different_v = 0;    //统计有几个不同的速度片
	for (int i = 0;i < NUM_OF_CAR;i++)
	{
		int v = Car[i].getmax_v();
		if (num_per_v.count(v) == 1)
			num_per_v[v]++;
		else
		{
			num_per_v.insert(pair<int, int>(v, 1));
			num_of_different_v++;
		}

	}
	int *perTimeDepart = new int[num_of_different_v];
	for (int i = 0;i < num_of_different_v;i++)
		perTimeDepart[i] = 999999;
	int num_car = num_per_v[cur_v];
	num_of_different_v = 0;
	while (NUM_OF_FINISH != NUM_OF_CAR)     //当还有车没有到达目的地，不断进行调度
	{
		//cout << "到目前的车辆数个数为" << num_car << endl;
		cur_time++;
		/*
		if (NUM_OF_DEPART == 2548)
		system("pause");
		if (NUM_OF_DEPART == 5175)
		system("pause");
		if (NUM_OF_DEPART == 7659)
		system("pause");
		*/
		...
		if (NUM_OF_DEPART == num_car&&NUM_OF_DEPART != NUM_OF_CAR)
		{
			perTimeDepart[num_of_different_v++] = cur_time;
			for (map<int, list<int>, greater<int>>::iterator pos = Different_v.begin();pos != Different_v.end();pos++)
			{
				if (pos->first == cur_v)
				{
					pos++;
					if (pos != Different_v.end())
					{
						cur_v = pos->first;
						depart_car_list.merge(Different_v[cur_v]);
						num_car += num_per_v[cur_v];
						break;
					}
					else
						pos--;
		...
		bool isOK = dispatchCarPerTime(cur_time, Cross, Road, Lane, Car, ThePathOfDrive, minLen2Nodes1, minLen2Nodes2, pathOf2Nodes1, pathOf2Nodes2, nodeToPath, depart_car_list, perTimeDepart);  //进行每个时间片的调度
		if (isOK == false)     //当出现死锁，返回，否则进行下一个时间片的调度
		{
			delete[]perTimeDepart;
			return -1;
		}

		//cout << "当前时刻进入车库的车辆数" << NUM_OF_FINISH << endl;
	}
	delete[]perTimeDepart;
...
int main(int argc, char *argv[])
{
	...
	ifstream infile;
	string line;
	map<int, list<int>, greater<int>>Different_v;
	...
	if (infile) // 有该文件  
	{
		while (getline(infile, line)) // line中不包括每行的换行符  
		{
			makeCar(Car, line, ThePathOfDrive, Different_v);
		}
	}
	else // 没有该文件  
	{
	...
	int value = dispatchCar(Cross, Road, Lane, Car, ThePathOfDrive, minLen2Nodes1, minLen2Nodes2, pathOf2Nodes1, pathOf2Nodes2, nodeToPath, Different_v);
	if (value > 0)
	{
		ofstream outfile;
