...
bool dispatchCarPerTime(int cur_time, vector<cross>&Cross, vector<road>&Road, int **Lane, vector<car>&Car, vector<vector<int>>&ThePathOfDrive, int *minLen2Nodes1, int *minLen2Nodes2, int *pathOf2Nodes1, int *pathOf2Nodes2, int *nodeToPath, list<int> &depart_car_list)
{
    ...
    */
	for (list<int>::iterator pos = depart_car_list.begin();pos != depart_car_list.end();)
	{
		...
		//if (NUM_ON_ROAD > 700)
		//	break;
		if (cur_time<74 && NUM_ON_ROAD >700)
			break;
		if (cur_time<156&& cur_time >74 && NUM_ON_ROAD > 750)
			break;
		if (cur_time<233 && cur_time > 156 && NUM_ON_ROAD > 800)
			break;
		if (cur_time > 233 && NUM_ON_ROAD > 850)
			break;
		if (num_depart > 200)
			break;
...
int dispatchCar(vector<cross>&Cross, vector<road>&Road, int **Lane, vector<car>&Car, vector<vector<int>>&ThePathOfDrive, int *minLen2Nodes1, int *minLen2Nodes2, int *pathOf2Nodes1, int *pathOf2Nodes2, int *nodeToPath,map<int, list<int>, greater<int>>&Different_v)  //一个时间片一个时间片进行相应的调度
{
	...
	while (NUM_OF_FINISH != NUM_OF_CAR)     //当还有车没有到达目的地，不断进行调度
	{
		...
		bool isOK = dispatchCarPerTime(cur_time, Cross, Road, Lane, Car, ThePathOfDrive, minLen2Nodes1, minLen2Nodes2, pathOf2Nodes1, pathOf2Nodes2, nodeToPath, depart_car_list);  //进行每个时间片的调度
