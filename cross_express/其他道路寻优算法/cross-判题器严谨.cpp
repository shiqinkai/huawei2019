...
void makeCar(vector<car>&Car, string A,hash_map<int,list<int>>&map, vector<vector<int>>&ThePathOfDrive)   //将car.txt导入到Car的Vector中。
{
	if (A[0] == '#')
		return;
	else
...
{                                               
	//cout << "do driveAllCarJustOnRoadToEndState1" << endl;
	...
	if (next_exp_d != 0)   
	{
		...
		for (int i = 0;i < next_num;i++)
		{
			...
			if (loc == next_lon - 1)    //表示该条车道已经满了
				continue;
			//cout << "last_sch=" << Road[next_road].getlast_sch(next_num*isFromNext + i) << endl;
			//if (next_road == 20)
			//{
			//	for (int i = 0;i < next_num*next_lon * 2;i++)
			//	{
			//		cout << Lane[20][i] << "  ";
			//		if (i%next_num == next_num - 1)
			//			cout << endl;
			//	}
			//}
			//cout << Lane[next_road][Road[next_road].getlast_sch(next_num*isFromNext + i)] << endl;
			//cout << "最后一辆车的位置" << loc << "  下一条道路的长度" << next_lon - 1 << "   期望的长度" << next_exp_d << "   最后一辆车的ID：" << Lane[next_road][Road[next_road].getlast_sch(next_num*isFromNext + i)] << endl;
			//cout << "当前操作=" << Car[Lane[next_road][Road[next_road].getlast_sch(next_num*isFromNext + i)] - first_car].getstate() << endl;
			if (loc != next_lon - 1)   //表示该条道路没满
			{
				...
				{
				//	cout << "can not drive car" << carId << endl;
					return 0;
				}
			}
...
void driveCar3(int carID, vector<car>&Car, int No_road,vector<road>&Road, int **Lane,int first_cross)  //对从车库出发的车进行调度，carID是绝对值，road是相对值
{
	int first_car = Car[0].getId();
	...
	//cout << "do drivecar3" << endl;
	//cout << "drive to the Road" << No_road << endl;
...
{
	//cout << "do driveAllCarForEachCross" << No_Cross << endl;
	...
	//cout << "the num of finish road of the cross is:" << Cross[No_Cross].getfinish_road() << endl;
	...
	{
	//	cout << "cur_road=" << cur_road <<"    cur_No"<<No<< endl;
		//cout << "num_of_driven=" << num_of_driven << endl;
		if (num_of_change > 30)
			return num_all_driven;  //当该道路内部出现循环时，跳出该节点，执行下一个节点
		if (num_of_driven == 4)
			break;                       //用来判断当四条道路没有可以移动的车辆时，退出当前路口的调度
		//if(cur_road!=-1)
		//	cout << "Road" << cur_road << " is finished:" << Road[cur_road].getis_finished(Road[cur_road].getisFrom()) << endl;
		...
		if (Road[cur_road].getlast_finish(isFrom1*num_road1 + first_sch1%num_road1) == (len1*isFrom1 - 1))      //表示该道路上该车前面没有车挡着
		{
			...
			//cout << "first_road=" << first_road << endl;
			//if (carID == 16723)
			//{
				//cout << "16723的路径为：";
			...
			for (pos = ThePathOfDrive[carID - first_car].begin();pos != (ThePathOfDrive[carID - first_car].end());pos++)
			{
				...
				if ((*pos) == cur_road + first_road)    //搜索到当前道路
				{
					...
					if (pos != (ThePathOfDrive[carID - first_car].end()))
					{
						...
						//cout << "the four roads are:" << arr1[0] << "  " << arr1[1] << "  " << arr1[2] << "  " << arr1[3] << endl;
						//cout << "cur_road=" << cur_road << "    next_road=" << next_r << endl;
					//	cout << "the car" << carID << "operate is:" << Car[carID - first_car].getoperate() << endl;
						break;
					}
					//if (pos != (ThePathOfDrive[carID - first_car].end())&&*pos == 5000)
					//	cout << "pos++=5000" << endl;
					//cout << " the end is:" << *(ThePathOfDrive[carID - first_car].end());	
					else
						pos--;
	...
	if (Cross[No_Cross].getfinish_road() == 4)
		//cout << "finish the Cross is:" << No_Cross << endl;
...
bool driveAllCarToTheEnd(vector<cross>&Cross, vector<road>&Road, int **Lane, vector<car>&Car, vector<vector<int>>&ThePathOfDrive)  //该函数对调度第二步的每个路口的遍历,如果进入死锁则返回false否则返回true
{
	...
	//cout << "do driveAllCarToTheEnd" << endl;
	int isd = Road[79].getisDouble();
	//cout << "the is_finished of the Road79 is";
	//for (int i = 0;i < isd + 1;i++)
	//{
	//	cout << Road[79].getis_finished(i) << "  ";
	//}
	//cout << endl;
	while (No_cross_finish < NUM_OF_CROSS)       //用于调度每个路口，直至路口调度完成
	{
		...
		//cout << No_cross_finish << "   " << NUM_OF_CROSS << endl;
...
{
	//cout << "do dispatchCarPerTime" << cur_time << endl;
	...
	for (int i = 0;i < NUM_OF_ROAD;i++)
	{
		...
		for (int j = 0;j < len;j++)
		{
			if (Lane[i][j] != -1)
				ontheroad++;
			if (cur_time == 5 && Lane[i][j] != -1)
			{
				outfile << Lane[i][j] << endl;
			}
		}
		//outfile << endl;
	}
	...
	cout << "调度第一次过后路上有的车辆数为：" << ontheroad << endl;
	int num34 = Road[34].getnum_road();
	int lon34 = Road[34].getlongth();
	int isD34 = Road[34].getisDouble();
	int len34 = num34*lon34*(1 + isD34);
	//cout << Car[6915].getstate() << endl;
	//cout << Car[6784].getstate() << endl;
	...
	cout << "调度第二次过后路上有的车辆数为：" << ontheroad << endl;
	for (int road = 0;road < NUM_OF_ROAD;road++)     //统计每条道路的拥挤程度
	{
		int isD = Road[road].getisDouble();
		int num_road = Road[road].getnum_road();
		int lon = Road[road].getlongth();
		int len = num_road*lon;
		int num_car1 = Road[road].getnum_car1();
		int num_car2 = Road[road].getnum_car2();
		float con1 = (float)num_car1 / (float)len;
		float con2 = (float)num_car2 / (float)len;
		Road[road].setcongestion1(con1);
		Road[road].setcongestion2(con2);
		if (Lane[road][len - 1] != -1)   //表示该道路已经满了 
			Road[road].setisFull(0, 1);
		if (isD == 1 && Lane[road][2 * len - 1])
			Road[road].setisFull(1, 1);
	}
	...
	{
		int act_time = Car[i].getact_time();
		if (act_time == cur_time)   //当时间片时间和车的实际发车时间相同时发车
		{
			int No_road = ThePathOfDrive[i].front()-first_road;
			driveCar3(i+first_car, Car, No_road, Road, Lane, first_cross);
		}
	}
	/*
	for (pos = Ready_start.begin();pos != Ready_start.end();)       //从发车队列中按照
	{
		...
		int from = Car[*pos-first_car].getfromId()-first_cross;
		int to = Car[*pos-first_car].gettoId() - first_cross;
		int v = Car[*pos-first_car].getmax_v();
		int drive_time;
		if (v > MIN_V_ROAD)
			drive_time = minLen2Nodes2[from*NUM_OF_CROSS + to];
		else
			drive_time = (minLen2Nodes1[from*NUM_OF_CROSS + to] / v);
		if (drive_time > 100)         //如果行驶时间太长，说明当前道路比较拥挤暂不发车
		{
			pos++;
			continue;
		}

		else                          //发车，并记录下当前车的路线，方便后面的operate的设置
		{
			Car[*pos-first_car].setact_time(cur_time);    //记录发车时间
			cout << "carID=" << (*pos) << "  road=";
			if (v > MIN_V_ROAD)                 //记录行车路线
			{
				...
				while (pathOf2Nodes2[i*NUM_OF_CROSS + j] != to)
				{
					...
					ThePathOfDrive[*pos - first_car].push_back(nodeToPath[i*NUM_OF_CROSS + k]);
					cout << nodeToPath[i*NUM_OF_CROSS + k] << "   ";
					i = k;
				}
				ThePathOfDrive[*pos - first_car].push_back(nodeToPath[i*NUM_OF_CROSS + j]);
				cout << nodeToPath[i*NUM_OF_CROSS + j] << endl;
			}
			else
			{
				int i = from;
		...
		driveCar3(*pos, Car, No_road, Road, Lane,first_cross);    //用来调度待发车的车
		cout << "the depart carId is:" << *pos << endl;
		pos = Ready_start.erase(pos);           //将已发车车辆从发车队列中除去
	}
	*/
	/*
	for (int i = 0;i < NUM_OF_ROAD;i++)
	{
		...
		int lon = Road[i].getlongth();
		int isD = Road[i].getisDouble();
		int len = num_road*lon*(1+isD);
		cout << "Road_N0=" << Road[i].getId() << endl;
		for (int j = 0;j < len;j++)
		{
			cout << Lane[i][j] << "  ";
			if (j%num_road == num_road-1)
				cout << endl;
		}
...
int dispatchCar(vector<cross>&Cross, vector<road>&Road, int **Lane, vector<car>&Car, vector<vector<int>>&ThePathOfDrive, hash_map<int, list<int>>&EachTimeOfPlan, int *minLen2Nodes1, int *minLen2Nodes2, int *pathOf2Nodes1, int *pathOf2Nodes2, int *nodeToPath)  //一个时间片一个时间片进行相应的调度
{
	int cur_time = -1;
	list<int> Ready_start;
	cout << NUM_OF_FINISH << "  " << NUM_OF_CAR << endl;
	cout << cur_time << endl;
	while (NUM_OF_FINISH != NUM_OF_CAR)     //当还有车没有到达目的地，不断进行调度
	{
		...
		cout << "已经发出的车辆数：" << NUM_OF_DEPART <<"应有"<<NUM_OF_DEPART+NUM_OF_REMAIN;
		cout << "在路上的车辆数为：" << ontheroad ;
		cout << "进入终点的车辆数：" << NUM_OF_FINISH ;
		cout << "现在有的所有车辆数：" << ontheroad + NUM_OF_FINISH << endl;
		//cout << (-1) / 4 << endl;
		if (cur_time == 1000)
			system("pause");
		//if (EachTimeOfPlan.count(cur_time) == 1)          //有预计当前触发的车，将这些车加入到发车队列中
...
int main()
{
	...
	infile.close();
	infile.open(AnswerTXT);
	if (infile) // 有该文件  
	{
		while (getline(infile, line)) // line中不包括每行的换行符  
		{
			makeAnswer(Car, line, ThePathOfDrive);
		}
	}
	else // 没有该文件  
	{
