...
int  driveAllCarForEachCross(int No_Cross, vector<cross>&Cross, vector<road>&Road, int **Lane, vector<car>&Car, vector<vector<int>>&ThePathOfDrive)   //此处是对于每个路口的车辆进行调度，No_Cross是相对值。
{
	...
	{
		if (num_of_change == 32)   //可以有效避免节点内由于道路调度进入局部死锁，导致有些道路无法遍历的问题
		{
			No = (No + 1) % 4;
			cur_road = arr[No];
	...
	*/
	for (list<int>::iterator pos = depart_car_list.begin();pos != depart_car_list.end();)
	{
		...
		*/
		//if (NUM_ON_ROAD > 700)
		//	break;
		if (cur_time<perTimeDepart[0] && NUM_ON_ROAD >400)
			break;
		if (cur_time<perTimeDepart[1] && cur_time >perTimeDepart[0] && NUM_ON_ROAD > 500)
			break;
		if (cur_time<perTimeDepart[2] && cur_time > perTimeDepart[1] && NUM_ON_ROAD > 600)
			break;
		if (cur_time > perTimeDepart[2] && NUM_ON_ROAD > 700)
			break;
		if (num_depart > 200)
			break;
