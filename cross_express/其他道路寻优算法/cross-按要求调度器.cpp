...
int driveCar3(int carID, vector<car>&Car, int No_road,vector<road>&Road, int **Lane,int first_cross)  //对从车库出发的车进行调度，carID是绝对值，road是相对值
{
	int first_car = Car[0].getId();
...
bool driveAllCarToTheEnd(vector<cross>&Cross, vector<road>&Road, int **Lane, vector<car>&Car, vector<vector<int>>&ThePathOfDrive)  //该函数对调度第二步的每个路口的遍历,如果进入死锁则返回false否则返回true
{
	...
	while (No_cross_finish < NUM_OF_CROSS)       //用于调度每个路口，直至路口调度完成
	{
		...
		if (No_cross_finish != NUM_OF_CROSS)
		{
			cout << "当前完成调度的节点个数为" << No_cross_finish;
			cout << "没有完成的路口的编号为：";
			for (int No_cross = 0;No_cross < NUM_OF_CROSS;No_cross++)    //一次对每个路口进行遍历
			{
				if (Cross[No_cross].getfinish_road() != 4)          //表示该路口已经调度完成
				{
					cout << No_cross <<"完成的道路调度数为："<<Cross[No_cross].getfinish_road()<<"    ";
				}
			}
			cout << endl;
		}
...
bool dispatchCarPerTime(int cur_time,vector<cross>&Cross, vector<road>&Road, int **Lane, vector<car>&Car,list<int>& Ready_start, vector<vector<int>>&ThePathOfDrive, int *minLen2Nodes1, int *minLen2Nodes2, int *pathOf2Nodes1, int *pathOf2Nodes2,int *nodeToPath,list<int>&remainToDepart)
{
	...
	{
		int first_car = Car[0].getId();
		cout << "13498的状态为" << Car[3498].getstate() << endl;
		for (int i = 0;i < NUM_OF_ROAD;i++)
		{
			int num_road = Road[i].getnum_road();
			int lon = Road[i].getlongth();
...
//	list<int>::iterator pos;
	for (pos = remainToDepart.begin();pos != remainToDepart.end();)
	{
		int No_road = ThePathOfDrive[*pos-first_car].front() - first_road;
		int Id= driveCar3(*pos, Car, No_road, Road, Lane, first_cross);
		if (Id == -1)
			pos = remainToDepart.erase(pos);
		else
			pos++;
	}
	for (int i = 0;i < NUM_OF_CAR;i++)
	{
		...
		if (act_time == cur_time)   //当时间片时间和车的实际发车时间相同时发车
		{
			...
			int Id=driveCar3(i+first_car, Car, No_road, Road, Lane, first_cross);
			if (Id != -1)
				remainToDepart.push_back(Id);
		}
	}
...
int dispatchCar(vector<cross>&Cross, vector<road>&Road, int **Lane, vector<car>&Car, vector<vector<int>>&ThePathOfDrive, hash_map<int, list<int>>&EachTimeOfPlan, int *minLen2Nodes1, int *minLen2Nodes2, int *pathOf2Nodes1, int *pathOf2Nodes2, int *nodeToPath)  //一个时间片一个时间片进行相应的调度
{
	...
	while (NUM_OF_FINISH != NUM_OF_CAR)     //当还有车没有到达目的地，不断进行调度
	{
		...
		if (cur_time == 1500)
			system("pause");
		bool isOK = dispatchCarPerTime(cur_time, Cross, Road, Lane, Car, Ready_start, ThePathOfDrive, minLen2Nodes1, minLen2Nodes2, pathOf2Nodes1, pathOf2Nodes2, nodeToPath, remainToDepart);  //进行每个时间片的调度
		cout << "isOK=" << isOK << endl;
