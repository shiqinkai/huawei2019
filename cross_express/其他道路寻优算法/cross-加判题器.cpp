...
{                                               
	...
	if (isD == 1)           //当是双车道时，需要对另一方向车道进行处理
	{
		...
		for (int i = len;i < 2 * len;i++)
		{
			...
			{
				int v = min(Car[Lane[No_road][i] - first].getmax_v(), Road[No_road].getmax_v());
				if (i - v*num < len || (last_one[i%num] >= (i - v*num) && Car[Lane[No_road][last_one[i%num]] - first].getstate() == 4)) //说明该车出路口或者前面有等待的车挡着
				{
					Car[Lane[No_road][i] - first].setstate(4);   //该车状态设置为等待
					last_one[i%num] = i;
				}
...
int driveCar1(int carId,int No_Cross,int first_cross,int cur_road, int next_road, vector<road>&Road, vector<car>&Car, int **Lane)   //该函数用于处理要过路口的车辆，更新相关参数
{
	...
	int cur_d = cur_sch / cur_num-cur_len_first;    //该车距离路口还有的距离
...
void driveCar3(int carID, vector<car>&Car, int No_road,vector<road>&Road, int **Lane,int first_cross)  //对从车库出发的车进行调度，carID是绝对值，road是相对值
{
	...
	for (int i = num_road*isFrom;i < num_road*(1 + isFrom);i++)    //按照调度算法决定驶入哪一条道路
	{
		...
		if (lastone != lon - 1)    //如果该条车道有空位置
		{
			...
			{
				int loc = lon*num_road*isFrom + num_road*(lon - v) + i;
...
int  driveAllCarForEachCross(int No_Cross, vector<cross>&Cross, vector<road>&Road, int **Lane, vector<car>&Car, vector<vector<int>>&ThePathOfDrive)   //此处是对于每个路口的车辆进行调度，No_Cross是相对值。
{
	...
	while (Cross[No_Cross].getfinish_road()<4)    //当当前路口还有道路没有调度完，否则跳出当前当前路口，进入下一个
	{
		...
		//if(carID==)
		int first_sch1 = Road[No].getfirst_sch(isFrom1);
		if (Road[cur_road].getlast_finish(isFrom1*num_road1 + first_sch1%num_road1) == (len1*isFrom1 - 1))      //表示该道路上该车前面没有车挡着
		{
			...
			if (Car[carID - first_car].gettoId() - first_cross == No_Cross)    //当当前车辆已经行驶到目的地
			{
				NUM_OF_FINISH++;
			...
			if (carID != -1)
			{
				int v = min(Car[carID - first_car].getmax_v(), Road[cur_road].getmax_v());
				int remain = Road[cur_road].getfirst_sch(isFrom1) / num_road1 - len1*isFrom1 + 1;
				if (remain >= v)    //当前面一辆车驶离后，该辆车不过路口，任在当前道路行驶
				{
			...
			else if ((Car[carID-first_car].getoperate()) == 1)     //表示当前的车为左转
			{
				...
				if (next_road1 != -1)    //考虑arr[(No + 3) % 4]这条道路是否为-1，如果不是-1进行判断
				{
					...
					int carID1 = getCarId(next_road1, Road, Car, Lane, isFrom_next, first_car);
			...
			else if ((Car[carID-first_car].getoperate()) == 2)     //表示当前的车为右转
			{
				...
				if (next_road1 != -1)
				{
					...
					int carID1 = getCarId(next_road1, Road, Car, Lane, isFrom_next1, first_car);
			   ...
			   if (next_road2 != -1)
				{
					...
					int carID2 = getCarId(next_road2, Road, Car, Lane, isFrom_next2, first_car);
					cout << "进入 2" << endl;
